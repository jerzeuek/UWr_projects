...
(define (div-operation) : (Value Value -> Answer)     ; dodanie nowej funkcji div-operation
  (λ (v1 v2)
    (type-case Value v1
      [(numV n1)
       (type-case Value v2
...
(define (op-num-num->proc [f : (Number Number -> Number)]) : (Value Value -> Answer) ; zmiana (Value Value -> Answer)
  (λ (v1 v2)
    (type-case Value v1
      [(numV n1)
       (type-case Value v2
         [(numV n2)
          (return (numV (f n1 n2)))]             ; zmiana na return 
         [else
          (err 'eval "type error")])]            ; zmiana na err 
      [else
       (err 'eval "type error")])))              ; zmiana na err

(define (op-num-bool->proc [f : (Number Number -> Boolean)]) : (Value Value -> Answer) ; zmiana (Value Value -> Answer)
  (λ (v1 v2)
    (type-case Value v1
      [(numV n1)
       (type-case Value v2
         [(numV n2)
          (return (boolV (f n1 n2)))]            ; zmiana na return
         [else
          (err 'eval "type error")])]            ; zmiana na err
      [else
       (err 'eval "type error")])))              ; zmiana na err

(define (op->proc [op : Op]) : (Value Value -> Answer)     ; zmiana (Value Value -> Answer)
  (type-case Op op
    [(add) (op-num-num->proc +)]
    [(sub) (op-num-num->proc -)]
    [(mul) (op-num-num->proc *)]
    [(div) (div-operation)]                      ; zmiana na div-operation
...
(define (eval [e : Exp]) : Answer
  (type-case Exp e
    ...
    [(opE o l r)
     (do ([v-l (eval l)]
          [v-r (eval r)])
       ((op->proc o) v-l v-r))]                  ; usunięcie return
    [(ifE b l r)
     (do ([v (eval b)])
       (type-case Value v
         [(boolV u)
