...
(define-type Exp
  ...
  (setE [x : Symbol] [e : Exp])
  (whileE [b : Exp] [e : Exp]))                          ;NOWE - dodanie whileE
  ...
  (cond
    ...
    [(s-exp-match? `{set! SYMBOL ANY} s)
     (setE (s-exp->symbol (second (s-exp->list s)))
           (parse (third (s-exp->list s))))]
    [(s-exp-match? `{while ANY ANY} s)                ;NOWE - dodanie reguły parsowania
  ...
  (type-case Exp e
    ...
    [(setE x e0)
     (begin 
       ...
       (voidV))]
    [(whileE b e)                         ;NOWE - dodanie do eval whileE
     (eval-while b e env)]))

(define (eval-while [b : Exp] [e : Exp] [env : Env]) : Value     ;NOWE - dodanie funkcji eval-while
  (type-case Value (eval b env)
    [(boolV v)
     (if v
         (begin
