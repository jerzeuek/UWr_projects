...
(module+ test
  ...
  (test (run `{{lambda {x} {+ x 1}} 5})
        (numV 6)))
;; fv  ———————————————————————————————————
(define (free-vars [s : S-Exp]) : (Listof Symbol) 
  (local 
    ((define (not-in xs)
       (local ((define (not-in-help xs v)
                 (cond [(empty? xs) #t]
                       [(equal? (first xs) v) #f]
                       [else (not-in-help (rest xs) v)])))
         (λ (v) (not-in-help xs v))))
     
     (define (free-vars-wparse [e : Exp]) : (Listof Symbol)
       (type-case Exp e
         [(numE n) empty]
         [(opE o l r) (let ([fvl (free-vars-wparse l)]) (append fvl
                                                             (filter (not-in fvl) (free-vars-wparse r))))]
         [(ifE b l r)
          (let ([fvb (free-vars-wparse b)]
                [fvl (free-vars-wparse l)]
                [fvr (free-vars-wparse r)]) (append fvb
                                                 (filter (not-in fvb) (append fvl
                                                                              (filter (not-in fvl) fvr)))))]
         [(varE x)
          (cons x empty)]
         [(letE x e1 e2)
          (let ([fve1 (free-vars-wparse e1)]) (append fve1
                                                   (filter (not-in (cons x fve1)) (free-vars-wparse e2))))]
         [(lamE x e)
          (filter (not-in (cons x empty)) (free-vars-wparse e))]

         [(appE e1 e2)
          (let ([fve1 (free-vars-wparse e1)]) (append fv

         )))
    (free-vars-wparse (parse s))))
