...
(define-type Exp
  ...
  (let*_E [l : (Listof (Symbol * Exp))] [e2 : Exp]))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{let ANY ANY} s)
     (letE (parse-list (s-exp->list (second (s-exp->list s))))
           (parse (third (s-exp->list s))))]
    [(s-exp-match? `{let* ANY ANY} s)
     (let*_E (parse-list (s-exp->list (second (s-exp->list s))))
             (parse (third (s-exp->list s))))]
...
(define (parse-list [ss : (Listof S-Exp)])
  ...
  (type-case Exp e
    ...
    [(let*_E l e2)
     (type-case (Listof (Symbol * Exp)) l
       [empty (eval e2 env)]
       [(cons p xs)
         (eval (letE xs e2) (extend-env env (fst p) (eval (snd p) env)))])]
    [(letE l e2)
     (let* ([xs (foldl (λ(x y) (cons (pair (fst x) (eval (snd x) env)) y)) empty l)]
            [new_env (foldl (λ(x y) (extend-env y (fst x) (snd x))) env xs)])
       (eval e2 new_env))]))
...
(module+ test
  ...
  (test (run `{let {{x 1}}
                {+ x {let {{x {+ x 1}}}
                       {* x 3}}}})
        (numV 7))
  (test (run `{let* {{x 1} {y x}}
                {+ y {let {{x {+ x 1}}}
                       {* x 3}}}})
        (numV 7))
  (test/exn (run `{let {{x 1} {y x}}
                {+ y {let {{x {+ x 1}}}
                       {* x 3}}}})
        "unbound variable"))
