...
(define (detranslate [e : ExpA] [v : Number]) : Exp
  (type-case ExpA e
    [(numA n)
     (numE n)]
    [(opA o l r)
     (opE o (detranslate l v) (detranslate r v))]
    [(ifA b l r)
     (ifE (detranslate b v)
          (detranslate l v)
          (detranslate r v))]
    [(varA x)
     (varE (string->symbol (generator-names (- v (+ x 1)))))]
    [(letA e1 e2)
     (letE
      (string->symbol (generator-names v))
      (detranslate e1 v)
      (detranslate e2 (+ v 1)))]))

(define (generator-names [x : Number]) : String
  (if (eq? x 0)
      ""
      (string-append "a" (generator-names (- x 1)))))
